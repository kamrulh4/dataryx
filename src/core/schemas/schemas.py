...
class RawLogInput(BaseModel):
    ...
    Attributes:
        dataryx_flow_id (int): The ID of the flow that generated the log.
...
class DataryxSettings(BaseModel):
    """Settings for dataryx serialization (YAML/JSON).
...
class DataryxNode(BaseModel):
    """Node representation for dataryx serialization (YAML/JSON)."""
...
class DataryxData(BaseModel):
    """Root model for dataryx serialization (YAML/JSON)."""

    dataryx_version: str
    dataryx_id: int
    dataryx_name: str
    dataryx_settings: DataryxSettings
    nodes: list[DataryxNode]
