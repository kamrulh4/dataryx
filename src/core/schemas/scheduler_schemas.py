...
class CronExpressionValidator:
    ...
    def validate(expression: str) -> bool:
        ...
        if len(parts) != 5:
            raise ValueError("Cron expression must have exactly 5 parts: minute hour day month day_of_week")
...
class ScheduledJobCreate(BaseModel):
    ...
    """
    name: str = Field(..., min_length=1, max_length=100, description="Name of the scheduled job")
    ...
    max_retries: int = Field(default=0, ge=0, le=5, description="Maximum retry attempts on failure")
    retry_delay_seconds: int = Field(default=60, ge=0, description="Delay between retries in seconds")
...
class ScheduledJobResponse(BaseModel):
    ...
    user_id: int
    cron_expression: str
    cron_human_readable: Optional[str] = Field(default=None, description="Human-readable cron description")
    timezone: str
    is_active: bool
    created_at: datetime
    updated_at: datetime
