"""
Main API router and endpoint definitions for the Dataryx application.
...
# Core modules
from core.auth.jwt import get_current_active_user, security
from core.configs import logger
from core.configs.node_store import check_if_has_default_setting, nodes_list
from core.database.connection import get_db, get_db_context
from core.database.models import CatalogNamespace, FlowRegistration, FlowRun
from core.configs.settings import AUTH_SERVICE_URL
import httpx

# File handling
from core.fileExplorer.funcs import (
    FileInfo,
    SecureFileExplorer,
    get_files_from_directory,
    validate_path_under_cwd,
)
from core.dataryx.analytics.analytics_processor import AnalyticsProcessor
...
from core.dataryx.extensions import get_instant_func_results
from core.dataryx.flow_graph import add_connection, delete_connection
from core.dataryx.sources.external_sources.sql_source.sql_source import create_sql_source_from_db_settings
from core.run_lock import get_flow_run_lock
from core.schemas import input_schema, output_model, schemas
from core.schemas.history_schema import HistoryActionType, HistoryState, OperationResponse, UndoRedoResult
from core.utils import excel_file_manager
from core.utils.fileManager import create_dir
from core.utils.utils import camel_case_to_snake_case
...
async def get_directory_contents(directory: str, file_types: list[str] = None,
    ...
    """
    from core.configs.settings import is_electron_mode
...
@router.get("/active_dataryx_sessions/", response_model=list[schemas.FlowSettings])
...
def _run_and_track(flow, user_id: int | None, token: str | None = None):
    """Wrapper that runs a flow and persists the run record to the database."""
    run_info = flow.run_graph()
    if run_info is None:
        return

    # Decrement VPS run count after a successful run
    if run_info.success and token is not None:
        try:
            with httpx.Client() as client:
                resp = client.post(
                    f"{AUTH_SERVICE_URL}/subscription/decrement",
                    headers={"Authorization": f"Bearer {token}"},
                    timeout=10.0
                )
                if resp.status_code != 200:
                    logger.warning(f"Could not decrement VPS run count: {resp.text}")
        except Exception as exc:
            logger.warning(f"Error connecting to VPS for run decrement: {exc}")

    # Persist run record
    try:
        # Build snapshot
        try:
            snapshot_data = flow.get_dataryx_data()
        ...
        with get_db_context() as db:
            # Try to find a matching registration for this flow path
            reg_id = None
...
async def run_flow(flow_id: int, background_tasks: BackgroundTasks,
                   ...
                   auth: Any = Depends(security)) -> JSONResponse:
    """Executes a flow in a background task (VPS Gated)."""
    ...
    # Gate: verify run allowed with VPS
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                sub_data = resp.json()
                if sub_data.get("is_expired"):
                    raise HTTPException(403, "Subscription expired")
                if sub_data.get("conversions_remaining", 0) <= 0:
                    raise HTTPException(403, "No runs remaining")
            else:
                raise HTTPException(resp.status_code, f"Auth verification failed: {resp.text}")
    ...
    async with lock:
        if flow.flow_settings.is_running:
            raise HTTPException(422, 'Flow is already running')
        background_tasks.add_task(_run_and_track, flow, user_id, token)
...
def add_node(flow_id: int, node_id: int, node_type: str, pos_x: int | float = 0,
    ...
    if node_type == 'explore_data':
        ...
        pre_snapshot = flow.get_dataryx_data() if flow.flow_settings.track_history else None
        ...
        if pre_snapshot is not None and flow.flow_settings.track_history:
            from core.schemas.history_schema import HistoryActionType
...
def get_expressions() -> list[str]:
    """Retrieves a list of all available Dataryx expression names."""
...
def get_list_of_saved_flows(path: str):
    """Scans a directory for saved flow files (`.dataryx`)."""
    try:
        # Validate path is within sandbox before proceeding
        explorer = SecureFileExplorer(
            start_path=storage.user_data_directory,
        ...
        if validated_path is None:
            return []
        return get_files_from_directory(str(validated_path), types=['dataryx'], sandbox_root=storage.user_data_directory)
    except:
        return []
