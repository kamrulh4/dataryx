...
from core import ServerRun, flow_file_handler
from core.auth.jwt import get_current_active_user, get_current_user_from_query
...
async def add_raw_log(raw_log_input: schemas.RawLogInput):
    ...
    flow = flow_file_handler.get_flow(raw_log_input.dataryx_flow_id)
    if not flow:
        raise HTTPException(status_code=404, detail="Flow not found")
...
async def stream_logs(flow_id: int, idle_timeout: int = 300, current_user=Depends(get_current_user_from_query)):
    ...
    """
    logger.info(f"Starting log stream for flow_id: {flow_id} by user: {current_user.email}")
