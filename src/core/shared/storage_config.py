"""
Centralized storage configuration for Dataryx.
...
def _is_docker_mode() -> bool:
    """Check if running in Docker mode based on DATARYX_MODE."""
    return os.environ.get("DATARYX_MODE") == "docker"


class DataryxStorage:
    """Centralized storage manager for Dataryx applications."""
    ...
    def base_directory(self) -> Path:
        """Get the base Dataryx storage directory (for internal container communication)."""
        if self._base_dir is None:
            if _is_docker_mode():
                # In Docker, internal storage stays inside /app
                base_path = os.environ.get("DATARYX_STORAGE_DIR", "/app/internal_storage")
            else:
                # Local development
                base_path = os.environ.get("DATARYX_STORAGE_DIR")
                if not base_path:
                    home_dir = Path.home()
                    base_path = home_dir / ".dataryx"
    ...
    def user_data_directory(self) -> Path:
        ...
        if self._user_data_dir is None:
            if _is_docker_mode():
                ...
                user_data_path = os.environ.get("DATARYX_USER_DATA_DIR", "/data/user")
            else:
    ...
    def flows_directory(self) -> Path:
        ...
        if _is_docker_mode():
            ...
            # Local development - flows in ~/.dataryx/flows
    ...
    def uploads_directory(self) -> Path:
        ...
        if _is_docker_mode():
            ...
            # Local development - uploads in ~/.dataryx/uploads
    ...
    def outputs_directory(self) -> Path:
        ...
        if _is_docker_mode():
            ...
            # Local development - outputs in ~/.dataryx/outputs
