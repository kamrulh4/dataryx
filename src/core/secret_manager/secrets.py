...
class SecureStorage:
    ...
    def __init__(self):
        env = os.environ.get("DATARYX_MODE")
        logger.debug(f"Using secure storage in {env} mode")
        if os.environ.get("DATARYX_MODE") == "electron":
...
def get_docker_secret_key() -> str | None:
    ...
    """
    env_key = os.environ.get("DATARYX_MASTER_KEY")
    if env_key:
        ...
        try:
            ...
            raise RuntimeError("DATARYX_MASTER_KEY is not a valid Fernet key")

    secret_path = "/run/secrets/dataryx_master_key"
...
def get_master_key():
    ...
    if os.environ.get("DATARYX_MODE") == "docker":
        ...
        if key is None:
            raise RuntimeError(
                "Master key not configured. Set DATARYX_MASTER_KEY environment variable "
                "or mount the dataryx_master_key Docker secret."
            )
        return key

    key = get_password("dataryx", "master_key")
    if not key:
        key = Fernet.generate_key().decode()
        set_password("dataryx", "master_key", key)
    return key
