...
def get_database_url():
    ...
    if os.environ.get("TESTING") == "True":
        ...
        test_db_path = storage.temp_directory / "test_dataryx.db"
        logger.debug(f"Using TESTING database URL: sqlite:///{test_db_path}")
        return f"sqlite:///{test_db_path}"

    custom_db_path = os.environ.get("DATARYX_DB_PATH")
    if custom_db_path:
        # logger.error("Using database URL:", os.environ.get("DATARYX_DB_URL"))
        return f"sqlite:///{custom_db_path}"
    # Use centralized location
    app_dir = get_app_data_dir()

    db_path = app_dir / "dataryx.db"
