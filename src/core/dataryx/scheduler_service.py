...
class SchedulerService:
    ...
    def _load_jobs_from_database(self):
        ...
        db = SessionLocal()
        try:
            active_jobs = db.query(db_models.ScheduledJob).filter(
                db_models.ScheduledJob.is_active == True
            ).all()
    ...
    def add_job(self, job_id: int, flow_id: int, cron_expression: str, timezone: str = "UTC"):
        """
...
async def handle_job_retry(job_id: int, max_retries: int, retry_delay: int):
    ...
    try:
        ...
        latest_run = db.query(db_models.JobRun).filter(
            db_models.JobRun.job_id == job_id
        ).order_by(db_models.JobRun.started_at.desc()).first()
        ...
        if latest_run and latest_run.attempt_number < max_retries:
            ...
            if job:
                logger.info(f"Retrying job {job_id}, attempt {latest_run.attempt_number + 1}")
...
async def execute_job_logic(job_id: int, flow_id: int):
    ...
    try:
        ...
        logger.info(f"Starting execution of flow {flow_id} (registration_id) for job {job_id}")
        
        # 1. Load the flow registration to get the path
        registration = db.query(db_models.FlowRegistration).filter(
            db_models.FlowRegistration.id == flow_id
        ).first()
        ...
        if run_info_obj and run_info_obj.success:
            ...
            job_run.error_message = "Flow execution failed or returned no success indicator"
        ...
        db.commit()
        logger.info(f"Successfully executed flow {flow_id} for job {job_id}. Status: {job_run.status}")
