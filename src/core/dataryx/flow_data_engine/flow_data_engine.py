...
from core.configs.flow_logger import NodeLogger

# Local imports - Flow File Components
from core.dataryx.flow_data_engine import utils
from core.dataryx.flow_data_engine.cloud_storage_reader import (
    CloudStorageReader,
    ensure_path_has_wildcard_pattern,
    get_first_file_from_s3_dir,
)
from core.dataryx.flow_data_engine.create import funcs as create_funcs
from core.dataryx.flow_data_engine.flow_file_column.main import (
    DataryxColumn,
    assert_if_dataryx_schema,
    convert_stats_to_column_info,
)
from core.dataryx.flow_data_engine.flow_file_column.utils import (
    cast_str_to_polars_type,
    get_polars_type,
    safe_eval_pl_type,
)
from core.dataryx.flow_data_engine.fuzzy_matching.prepare_for_fuzzy_match import prepare_for_fuzzy_match
from core.dataryx.flow_data_engine.join import (
...
from core.dataryx.flow_data_engine.sample_data import create_fake_data
...
from core.dataryx.flow_data_engine.threaded_processes import write_threaded
from core.dataryx.sources.external_sources.base_class import ExternalDataSource
from core.schemas import cloud_storage_schemas, input_schema
from core.schemas import transform_schema as transform_schemas
from core.schemas.schemas import ExecutionLocationsLiteral, get_global_execution_location
from core.utils.utils import ensure_similarity_dicts
...
class FlowDataEngine:
    """The core data handling engine for Dataryx.
    ...
    Attributes:
        ...
        _schema: A cached list of `DataryxColumn` objects representing the schema.
    """
    ...
    errors: list = None
    _schema: list[DataryxColumn] | None = None
    ...
    def __init__(
        ...
        schema: list[DataryxColumn] | list[str] | pl.Schema = None,
        number_of_records: int = None,
        ...
        Args:
            ...
            schema: An optional schema definition. Can be a list of `DataryxColumn` objects,
    ...
    def _handle_raw_data_format(self, raw_data: input_schema.RawData):
        ...
        """
        dataryx_schema = list(DataryxColumn.create_from_minimal_field_info(c) for c in raw_data.columns)
        polars_schema = pl.Schema(
            [
                (dataryx_column.column_name, dataryx_column.get_polars_type().pl_datatype)
                for dataryx_column in dataryx_schema
            ]
        )
        try:
    ...
    def _finalize_initialization(
        ...
        if assert_if_dataryx_schema(schema):
            self._schema = schema
    ...
    def data_frame(self, df: pl.LazyFrame | pl.DataFrame):
        ...
        if self.lazy and isinstance(df, pl.DataFrame):
            raise Exception("Cannot set a non-lazy dataframe to a lazy dataryx")
        self._data_frame = df
        self._schema = None
    ...
    @property
    def schema(self) -> list[DataryxColumn]:
        """The schema of the DataFrame as a list of `DataryxColumn` objects.

        This property lazily calculates the schema if it hasn't been determined yet.

        Returns:
            A list of `DataryxColumn` objects describing the schema.
        """
    ...
    @classmethod
    def create_from_schema(cls, schema: list[DataryxColumn]) -> FlowDataEngine:
        """Creates an empty FlowDataEngine from a schema definition.

        Args:
            schema: A list of `DataryxColumn` objects defining the schema.
    ...
    def _handle_schema(
        self, schema: list[DataryxColumn] | list[str] | pl.Schema | None, pl_schema: pl.Schema
    ) -> list[DataryxColumn] | None:
        ...
        if schema is None and pl_schema is not None:
            ...
            return None
        elif assert_if_dataryx_schema(schema) and pl_schema is None:
            return schema
    ...
    def _handle_polars_schema(self, schema: pl.Schema, pl_schema: pl.Schema) -> list[DataryxColumn]:
        """Handles Polars schema conversion."""
        flow_file_columns = [
            DataryxColumn.create_from_polars_dtype(column_name=col_name, data_type=dtype)
    ...
    def _handle_string_schema(self, schema: list[str], pl_schema: pl.Schema) -> list[DataryxColumn]:
    ...
    def do_pivot(self, pivot_input: transform_schemas.PivotInput, node_logger: NodeLogger = None) -> FlowDataEngine:
        ...
        if len(pivot_input.index_columns) == 0:
            ...
            ff = self.apply_dataryx_formula("1", col_name="__temp__")
        else:
            no_index_cols = False
            ff = self
    ...
    def _add_grouped_record_id(self, record_id_settings: transform_schemas.RecordIdInput) -> FlowDataEngine:
        ...
        output_schema = [DataryxColumn.from_input(record_id_settings.output_column_name, "UInt64")]
    ...
    def get_schema_column(self, col_name: str) -> DataryxColumn:
        ...
        Returns:
            A `DataryxColumn` object for the specified column, or `None` if not found.
        """
    ...
    def make_unique(self, unique_input: transform_schemas.UniqueInput = None) -> FlowDataEngine:
        ...
        """
        if unique_input is None or not unique_input.columns:
            strategy = unique_input.strategy if unique_input else "any"
            return FlowDataEngine(self.data_frame.unique(keep=strategy))
...
def execute_polars_code(*dataryx_tables: FlowDataEngine, code: str) -> FlowDataEngine:
    ...
    Args:
        *dataryx_tables: A variable number of `FlowDataEngine` objects to be
    ...
    """
    polars_executable = polars_code_parser.get_executable(code, num_inputs=len(dataryx_tables))
    if len(dataryx_tables) == 0:
        kwargs = {}
    elif len(dataryx_tables) == 1:
        kwargs = {"input_df": dataryx_tables[0].data_frame}
    else:
        kwargs = {f"input_df_{i+1}": dataryx_table.data_frame for i, dataryx_table in enumerate(dataryx_tables)}
