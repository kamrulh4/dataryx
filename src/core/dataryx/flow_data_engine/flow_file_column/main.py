...
from core.dataryx.flow_data_engine.flow_file_column.interface import DataTypeGroup, ReadableDataTypeGroup
from core.dataryx.flow_data_engine.flow_file_column.polars_type import PlType
from core.dataryx.flow_data_engine.flow_file_column.type_registry import convert_pl_type_to_string
...
class DataryxColumn:
    ...
    def __repr__(self):
        ...
        """
        return (
            f"DataryxColumn(name='{self.column_name}', "
    ...
    def __str__(self):
        ...
        """
        # --- Header (Always Shown) ---
        header = f"<DataryxColumn: '{self.column_name}'>"
        lines = []
    ...
    @classmethod
    def create_from_polars_type(cls, polars_type: PlType, **kwargs) -> "DataryxColumn":
    ...
    @classmethod
    def from_input(cls, column_name: str, data_type: str, **kwargs) -> "DataryxColumn":
    ...
    @classmethod
    def create_from_minimal_field_info(cls, minimal_field_info: input_schema.MinimalFieldInfo) -> "DataryxColumn":
...
def convert_stats_to_column_info(stats: list[dict]) -> list[DataryxColumn]:
    return [DataryxColumn.create_from_polars_type(PlType(**c)) for c in stats]


def convert_pl_schema_to_raw_data_format(pl_schema: pl.Schema) -> list[input_schema.MinimalFieldInfo]:
    return [
        DataryxColumn.create_from_polars_type(PlType(column_name=k, pl_datatype=v)).get_minimal_field_info()
        for k, v in pl_schema.items()
    ]


def assert_if_dataryx_schema(obj: Iterable) -> bool:
    """
    Assert that the object is a valid iterable of DataryxColumn objects.
    """
    if isinstance(obj, (list, set, tuple)):
        return all(isinstance(item, DataryxColumn) for item in obj)
    return False
