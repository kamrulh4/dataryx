...
from core.dataryx.analytics.utils import create_graphic_walker_node_from_node_promise
...
from core.dataryx.flow_data_engine.cloud_storage_reader import CloudStorageReader
from core.dataryx.flow_data_engine.flow_data_engine import FlowDataEngine, execute_polars_code
from core.dataryx.flow_data_engine.flow_file_column.main import DataryxColumn, cast_str_to_polars_type
from core.dataryx.flow_data_engine.polars_code_parser import polars_code_parser
from core.dataryx.flow_data_engine.read_excel_tables import (
...
from core.dataryx.flow_node.schema_utils import create_schema_callback_with_output_config
from core.dataryx.graph_tree.graph_tree import (
    add_un_drawn_nodes,
    build_flow_paths,
    build_node_info,
    calculate_depth,
...
from core.dataryx.node_designer.custom_node import CustomNodeBase
from core.dataryx.schema_callbacks import calculate_fuzzy_match_schema, pre_calculate_pivot_schema
from core.dataryx.sources import external_sources
from core.dataryx.sources.external_sources.factory import data_source_factory
from core.dataryx.sources.external_sources.sql_source import models as sql_models
from core.dataryx.sources.external_sources.sql_source import utils as sql_utils
from core.dataryx.sources.external_sources.sql_source.sql_source import BaseSqlSource, SqlSource
from core.dataryx.filter_expressions import build_filter_expression
from core.dataryx.util.calculate_layout import calculate_layered_layout
from core.dataryx.util.execution_orderer import ExecutionPlan, ExecutionStage, compute_execution_plan
from core.dataryx.utils import snake_case_to_camel_case
from core.schemas import input_schema, schemas, transform_schema
...
from core.schemas.history_schema import HistoryActionType, HistoryState, UndoRedoResult
from core.schemas.output_model import NodeData, NodeResult, RunInformation
from core.schemas.transform_schema import FuzzyMatchInputManager
from core.secret_manager.secret_manager import decrypt_secret, get_encrypted_secret
from core.utils.arrow_reader import get_read_top_n
...
def with_history_capture(action_type: "HistoryActionType", description_template: str = "Update {node_type} settings"):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(self: "FlowGraph", *args, **kwargs):
            ...
            pre_snapshot = self.get_dataryx_data()
...
def get_xlsx_schema(
    ...
    Returns:
        A list of DataryxColumn objects representing the schema.
    """
    try:
...
class FlowGraph:
    ...
    schema: list[DataryxColumn] | None = None
    ...
    def __init__(
        ...
        from core.dataryx.history_manager import HistoryManager
        from core.schemas.history_schema import HistoryConfig
    ...
    def capture_history_if_changed(
        self,
        pre_snapshot: schemas.DataryxData,
        action_type: HistoryActionType,
        description: str,
        node_id: int = None,
    ) -> bool:
        ...
        Args:
            pre_snapshot: The DataryxData captured BEFORE the change.
    ...
    def restore_from_snapshot(self, snapshot: schemas.DataryxData) -> None:
        """Clear current state and rebuild from a snapshot.

        This method is used internally by undo/redo to restore a previous state.

        Args:
            snapshot: The DataryxData snapshot to restore from.
        """
        from core.dataryx.manage.io_dataryx import (
            _dataryx_data_to_flow_information,
        ...
        flow_info = _dataryx_data_to_flow_information(snapshot)
    ...
    def add_union(self, union_settings: input_schema.NodeUnion):
        ...
        def _func(*dataryx_tables: FlowDataEngine):
            dfs: list[pl.LazyFrame] | list[pl.DataFrame] = [flt.data_frame for flt in dataryx_tables]
    ...
    def add_group_by(self, group_by_settings: input_schema.NodeGroupBy):
        ...
        def schema_callback():
            ...
            for old_name, new_name, data_type in output_columns:
                ...
                output_schema.append(DataryxColumn.from_input(data_type=data_type, column_name=new_name))
    ...
    def add_polars_code(self, node_polars_code: input_schema.NodePolarsCode):
        ...
        def _func(*dataryx_tables: FlowDataEngine) -> FlowDataEngine:
            return execute_polars_code(*dataryx_tables, code=node_polars_code.polars_code_input.polars_code)
    ...
    def add_formula(self, function_settings: input_schema.NodeFormula):
        ...
        if output_type not in (None, transform_schema.AUTO_DATA_TYPE):
            new_col = [
                DataryxColumn.from_input(column_name=function_settings.function.field.name, data_type=str(output_type))
            ]
        else:
            new_col = [DataryxColumn.from_input(function_settings.function.field.name, "String")]
    ...
    def add_node_step(
        ...
        output_schema: list[DataryxColumn] = None,
        node_type: str = None,
    ...
    def add_cloud_storage_writer(self, node_cloud_storage_writer: input_schema.NodeCloudStorageWriter) -> None:
        ...
        def _func(df: FlowDataEngine):
            ...
            if execute_remote:
                settings = get_cloud_storage_write_settings_worker_interface(
                    ...
                    lf=df.data_frame,
                    user_id=node_cloud_storage_writer.user_id,
                    dataryx_node_id=node_cloud_storage_writer.node_id,
                    dataryx_flow_id=self.flow_id,
                )
        ...
        def schema_callback():
            ...
            if self.get_node(node_cloud_storage_writer.node_id).is_correct:
                ...
                return [DataryxColumn.from_input(column_name="__error__", data_type="String")]
    ...
    def add_external_source(self, external_source_input: input_schema.NodeExternalSource):
        ...
        if external_source_input.source_settings.fields and len(external_source_input.source_settings.fields) > 0:
            ...
            def schema_callback():
                return [
                    DataryxColumn.from_input(f.name, f.data_type) for f in external_source_input.source_settings.fields
                ]
        ...
        if node:
            ...
            if start_hash != node.hash:
                ...
                if len(received_file.fields) > 0:
                    ...
                    def schema_callback():
                        return [DataryxColumn.from_input(f.name, f.data_type) for f in received_file.fields]
    ...
    def run_graph(self) -> RunInformation | None:
        ...
        try:
            ...
            self.flow_settings.is_canceled = False
            self.flow_logger.clear_log_file()
            self.flow_logger.info("Starting to run Dataryx flow...")
            execution_plan = compute_execution_plan(
                nodes=self.nodes, flow_starts=self._flow_starts + self.get_implicit_starter_nodes()
            )
    ...
    def get_dataryx_data(self) -> schemas.DataryxData:
        ...
        for node in self.nodes:
            ...
            nodes.append(dataryx_node)

        settings = schemas.DataryxSettings(
        ...
        return schemas.DataryxData(
            dataryx_version=__version__,
            dataryx_id=self.flow_id,
            dataryx_name=self.__name__,
            dataryx_settings=settings,
            nodes=nodes,
        )
        ...
        try:
            if suffix == ".dataryx":
                raise DeprecationWarning(
                    "The .dataryx format is deprecated. Please use .yaml or .json formats.\n\n"
                    "Or stay on v0.4.1 if you still need .dataryx support.\n\n"
                )
            elif suffix in (".yaml", ".yml"):
                dataryx_data = self.get_dataryx_data()
