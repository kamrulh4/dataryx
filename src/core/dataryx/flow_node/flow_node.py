...
from core.configs import logger, node_store
from core.configs.flow_logger import NodeLogger
from core.dataryx.flow_data_engine.flow_data_engine import FlowDataEngine
from core.dataryx.flow_data_engine.flow_file_column.main import DataryxColumn
from core.dataryx.flow_data_engine.subprocess_operations import (
    ExternalCloudWriter,
...
from core.dataryx.flow_node.executor import NodeExecutor
...
from core.dataryx.flow_node.output_field_config_applier import apply_output_field_config
from core.dataryx.flow_node.schema_callback import SingleExecutionFuture
from core.dataryx.flow_node.schema_utils import create_schema_callback_with_output_config
from core.dataryx.flow_node.state import NodeExecutionState
from core.dataryx.setting_generator import setting_generator, setting_updator
from core.dataryx.utils import get_hash
from core.schemas import input_schema, schemas
from core.schemas.output_model import FileColumn, NodeData, TableExample
...
class FlowNode:
    ...
    def create_schema_callback_from_function(self, f: Callable) -> Callable[[], list[DataryxColumn]]:
        ...
        def schema_callback() -> list[DataryxColumn]:
            try:
    ...
    def get_flow_file_column_schema(self, col_name: str) -> DataryxColumn | None:
        ...
        Returns:
            The DataryxColumn object for that column, or None if not found.
        """
        for s in self.schema:
            if s.column_name == col_name:
                return s

    def get_predicted_schema(self, force: bool = False) -> list[DataryxColumn] | None:
        ...
        Returns:
            A list of DataryxColumn objects representing the predicted schema.
        """
        logger.info(
    ...
    def _predicted_data_getter(self) -> FlowDataEngine | None:
        ...
        try:
            ...
            logger.warning("there was an issue with the function, returning an empty Dataryx result")
            logger.warning(e)
    ...
    def schema(self) -> list[DataryxColumn]:
        ...
        Returns:
            A list of DataryxColumn objects.
        """
        try:
