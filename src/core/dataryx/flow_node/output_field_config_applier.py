...
from core.schemas.input_schema import OutputFieldConfig, OutputFieldInfo
...
def _apply_raise_on_missing(
    ...
    Args:
        dataryx_engine: Input flow data engine
    ...
    missing_columns = set(cols) - set(dataryx_engine.columns)
    if missing_columns:
        raise ValueError(f"Missing required columns: {', '.join(sorted(missing_columns))}")
    if dataryx_engine.columns != cols:
        return FlowDataEngine(_select_columns_in_order(dataryx_engine.data_frame, fields))
    else:
        return dataryx_engine
...
def _validate_data_types(df: FlowDataEngine, fields: list[OutputFieldInfo]) -> None:
    ...
    schema: dict[str, DataryxColumn] = {column.column_name: column for column in df.schema}
    mismatches = []
    for field in fields:

        if field.name not in schema:
            continue
        # Use DataryxColumn infrastructure to convert dtype to string
