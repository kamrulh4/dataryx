...
from core.dataryx.node_designer.ui_components import (
    DataryxInComponent,
    IncomingColumns,
    SecretSelector,
    Section,
)
from core.schemas.schemas import NodeTemplate, NodeTypeLiteral, TransformTypeLiteral
...
def to_frontend_schema(model_instance: BaseModel) -> dict:
    ...
    nested `Section` and `DataryxInComponent` instances.
...
def _convert_value(value: Any) -> Any:
    ...
    elif isinstance(value, DataryxInComponent):
...
def create_section(**components: DataryxInComponent) -> Section:
    """
    ...
    Args:
        **components: Keyword arguments where each key is the component name
                      and the value is a `DataryxInComponent` instance.
...
class NodeSettings(BaseModel):
    ...
    def get_value(self, field_name: str) -> Any:
        ...
        if field_name in self.model_fields:
            ...
            if component is not None:
                if isinstance(component, DataryxInComponent):
    ...
    def get_all_components(self) -> dict[str, DataryxInComponent]:
        """
        Returns all UI components in the settings, including those nested in sections.

        Returns:
            Dictionary mapping field names to their DataryxInComponent instances.
        """
        components = {}
        ...
        for field_name in self.model_fields:
            ...
            if isinstance(value, DataryxInComponent):
...
class SectionBuilder:
    ...
    def add_component(self, name: str, component: DataryxInComponent) -> "SectionBuilder":
...
class CustomNodeBase(BaseModel):
    """
    The base class for creating a custom node in Dataryx.
    ...
    def get_secret_names(self) -> list[str]:
        ...
        """
        from core.dataryx.node_designer.ui_components import SecretSelector
