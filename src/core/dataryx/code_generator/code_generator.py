...
from core.dataryx.util.execution_orderer import determine_execution_order
...
class FlowGraphToPolarsConverter:
    ...
    that uses only Polars, without any Dataryx dependencies.
    """
    ...
    def _handle_cloud_storage_reader(
        ...
        self.imports.add("import dataryx as ff")
    ...
    @staticmethod
    def _generate_pl_schema_with_typing(dataryx_schema: list[DataryxColumn]) -> str:
        polars_schema_str = (
            "pl.Schema(["
            + ", ".join(
                f'("{dataryx_column.column_name}", pl.{dataryx_column.data_type})'
    ...
    def get_manual_schema_input(self, dataryx_schema: list[DataryxColumn]) -> str:
        polars_schema_str = self._generate_pl_schema_with_typing(dataryx_schema)
        is_valid_pl_schema = self._validate_pl_schema(polars_schema_str)
        if is_valid_pl_schema:
            return polars_schema_str
        else:
            return "[" + ", ".join([f'"{c.name}"' for c in dataryx_schema]) + "]"
    ...
    def _handle_manual_input(
        ...
        dataryx_schema = list(
            DataryxColumn.create_from_minimal_field_info(c) for c in settings.raw_data_format.columns
        )
        schema = self.get_manual_schema_input(dataryx_schema)
    ...
    def _handle_user_defined(
        ...
        if class_name not in self.custom_node_classes:
            ...
            self.imports.add("from core.dataryx.node_designer import CustomNodeBase, Section, NodeSettings, SingleSelect, MultiSelect, IncomingColumns, ColumnSelector, NumericInput, TextInput, DropdownSelector, TextArea, Toggle")
    ...
    def _parse_filter_expression(self, expr: str) -> str:
        """Parse Dataryx filter expression to Polars expression."""
    ...
    def _create_basic_filter_expr(self, basic: transform_schema.BasicFilter) -> str:
        ...
        """
        from core.schemas.transform_schema import FilterOperator
    ...
    def _get_polars_dtype(self, dtype_str: str) -> str:
        """Convert Dataryx dtype string to Polars dtype."""
        dtype_map = {
            "String": "pl.Utf8",
            "Integer": "pl.Int64",
            "Double": "pl.Float64",
    ...
    def _build_final_code(self) -> str:
        ...
        lines.append("    Generated from Dataryx")
        lines.append('    """')
        lines.append("    ")
