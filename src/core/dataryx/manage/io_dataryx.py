...
from core.dataryx.manage.compatibility_enhancements import ensure_compatibility, load_dataryx_pickle
from core.schemas import input_schema, schemas
...
def _validate_flow_path(flow_path: Path) -> Path:
    ...
    # Check extension
    allowed_extensions = {".yaml", ".yml", ".json", ".dataryx"}
...
def _derive_connections_from_nodes(nodes: list[schemas.DataryxNode]) -> list[tuple[int, int]]:
...
def _load_dataryx_yaml(flow_path: Path) -> schemas.FlowInformation:
    """
    Load a dataryx from YAML format and convert to FlowInformation.
    ...
    # Load as DataryxData first (handles setting_input validation via node type)
    dataryx_data = schemas.DataryxData.model_validate(data)
    # Convert to FlowInformation
    return _dataryx_data_to_flow_information(dataryx_data)


def _load_dataryx_json(flow_path: Path) -> schemas.FlowInformation:
    """
    Load a dataryx from JSON format and convert to FlowInformation.
...
def _dataryx_data_to_flow_information(dataryx_data: schemas.DataryxData) -> schemas.FlowInformation:
    from core.schemas.schemas import get_settings_class_for_node_type

    nodes_dict = {}
    node_starts = []
    for node in dataryx_data.nodes:
        setting_input = None
        if node.setting_input is not None:
            ...
            setting_data["flow_id"] = dataryx_data.dataryx_id
    ...
    connections = _derive_connections_from_nodes(dataryx_data.nodes)

    flow_settings = schemas.FlowSettings(
        flow_id=dataryx_data.dataryx_id,
        name=dataryx_data.dataryx_name,
        description=dataryx_data.dataryx_settings.description,
        execution_mode=dataryx_data.dataryx_settings.execution_mode,
        execution_location=dataryx_data.dataryx_settings.execution_location,
        auto_save=dataryx_data.dataryx_settings.auto_save,
        show_detailed_progress=dataryx_data.dataryx_settings.show_detailed_progress,
        max_parallel_workers=dataryx_data.dataryx_settings.max_parallel_workers,
    )

    return schemas.FlowInformation(
        flow_id=dataryx_data.dataryx_id,
        flow_name=dataryx_data.dataryx_name,
...
def _load_flow_storage(flow_path: Path) -> schemas.FlowInformation:
    ...
    Supports:
    - .dataryx (pickle) - legacy format
    - .yaml / .yml - new YAML format
    - .json - JSON format

    Args:
        flow_path: Path to the dataryx
    ...
    if suffix == ".dataryx":
        try:
            flow_storage_obj = load_dataryx_pickle(str(flow_path))
            ensure_compatibility(flow_storage_obj, str(flow_path))
            return flow_storage_obj
        except Exception as e:
            raise ValueError(
                f"Failed to open legacy .dataryx: {e}\n\n" f"Try migrating: migrate_dataryx('{flow_path}')"
            ) from e

    elif suffix in (".yaml", ".yml"):
        return _load_dataryx_yaml(flow_path)

    elif suffix == ".json":
        return _load_dataryx_json(flow_path)
    else:
...
def open_flow(flow_path: Path, user_id: int | None = None) -> FlowGraph:
    """
    Open a dataryx from a given path.

    Supports multiple formats:
    - .dataryx (pickle) - legacy format, auto-migrated
    - .yaml / .yml - new YAML format
    - .json - JSON format

    Args:
        flow_path (Path): The absolute or relative path to the dataryx
        user_id (int | None): The ID of the user importing the flow, used to resolve cloud connections.
    Returns:
        FlowGraph: The dataryx object
    """
