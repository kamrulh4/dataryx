"""
Compatibility enhancements for opening old dataryx versions.
...
class LegacyUnpickler(pickle.Unpickler):
    ...
    When loading old .dataryx pickles, transform_schema classes were dataclasses.
...
def load_dataryx_pickle(path: str) -> Any:
    """
    Load a dataryx pickle using legacy-compatible unpickling.

    This handles old dataryxs where transform_schema classes were dataclasses
...
def ensure_compatibility(flow_storage_obj: schemas.FlowInformation, flow_path: str):
    """
    Main compatibility function - migrates old dataryx schemas to current version.
...
def load_and_migrate_dataryx(flow_path: str) -> schemas.FlowInformation:
    """
    Convenience function: Load a dataryx and apply all compatibility migrations.

    Args:
        flow_path: Path to the .dataryx pickle

    Returns:
        Fully migrated FlowInformation object
    """
    flow_storage_obj = load_dataryx_pickle(flow_path)
