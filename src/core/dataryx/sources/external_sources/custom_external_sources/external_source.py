...
class CustomExternalSource(ExternalDataSource):
    ...
    @staticmethod
    def parse_schema(schema: list[Any]) -> list[DataryxColumn]:
        if len(schema) == 0:
            return []
        first_col = schema[0]
        if isinstance(first_col, dict):
            return [DataryxColumn(**col) for col in schema]
        elif isinstance(first_col, (list, tuple)):
            return [DataryxColumn.from_input(column_name=col[0], data_type=col[1]) for col in schema]
        elif isinstance(first_col, str):
            return [DataryxColumn.from_input(column_name=col, data_type="varchar") for col in schema]
        elif isinstance(first_col, input_schema.MinimalFieldInfo):
            return [DataryxColumn.from_input(column_name=col.name, data_type=col.data_type) for col in schema]
        elif isinstance(first_col, DataryxColumn):
            return schema
        else:
