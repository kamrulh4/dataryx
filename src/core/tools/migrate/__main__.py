#!/usr/bin/env python
"""
Dataryx Migration Tool - CLI Entry Point
...
Examples:
    ...
    python -m tools.migrate my_flow.dataryx

    # Migrate to specific output
    python -m tools.migrate my_flow.dataryx -o my_flow.yaml

    # Migrate entire directory
    python -m tools.migrate ./flows/

    # Migrate to JSON instead of YAML
    python -m tools.migrate my_flow.dataryx --format json
...
from tools.migrate.migrate import migrate_directory, migrate_dataryx


def main():
    parser = argparse.ArgumentParser(
        prog="dataryx-migrate",
        description="Migrate old .dataryx pickles to YAML format",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s my_flow.dataryx              Migrate single file to YAML
  %(prog)s ./flows/                      Migrate all files in directory
  %(prog)s flow.dataryx -o flow.yaml    Specify output path
  %(prog)s ./flows/ --format json        Output as JSON instead of YAML
        """,
    )

    parser.add_argument("path", type=Path, help="Path to .dataryx or directory containing .dataryx files")
    ...
    if args.dry_run:
        if args.path.is_file():
            ...
            dataryxs = list(args.path.glob("**/*.dataryx"))
            print(f"Would migrate {len(dataryxs)} file(s):")
            for f in dataryxs:
                print(f"  - {f}")
        sys.exit(0)
    ...
    try:
        if args.path.is_file():
            migrate_dataryx(args.path, args.output, args.format)
