"""
Migration logic for converting old dataryx pickles to new YAML format.
"""
...
class LegacyUnpickler(pickle.Unpickler):
    ...
    # These are all from core/schemas/transform_schema.py
...
def load_legacy_dataryx(path: Path) -> Any:
    """
    Load an old dataryx using legacy class definitions.

    Args:
        path: Path to the .dataryx pickle
...
def transform_to_new_schema(data: dict) -> dict:
    """
    Transform the legacy schema structure to the new DataryxData format.

    This handles:
    - ReceivedTable: flat fields -> nested table_settings
    - OutputSettings: separate table fields -> unified table_settings
    - Field name changes (flow_id -> dataryx_id, etc.)

    Args:
        data: Dict representation of legacy FlowInformation

    Returns:
        Transformed dict ready for YAML serialization (DataryxData format)
    """
    node_starts = set(data.get("node_starts", []))

    result = {
        "dataryx_version": "2.0",
        "dataryx_id": data.get("flow_id", 1),
        "dataryx_name": data.get("flow_name", ""),
        "dataryx_settings": _transform_flow_settings(data.get("flow_settings", {})),
...
def _transform_flow_settings(settings: dict) -> dict:
    """Transform flow settings to DataryxSettings format."""
    if not settings:
        return {
...
def _transform_nodes(nodes_data: dict, node_starts: set) -> list[dict]:
    """Transform nodes dict to DataryxNode list format."""
    nodes = []
...
def migrate_dataryx(input_path: Path, output_path: Path = None, format: str = "yaml") -> Path:
    """
    Migrate a single dataryx from pickle to YAML format.

    Args:
        input_path: Path to the .dataryx pickle
    ...
    # Load legacy dataryx
    legacy_data = load_legacy_dataryx(input_path)
...
def migrate_directory(dir_path: Path, output_dir: Path = None, format: str = "yaml") -> list[Path]:
    """
    Migrate all dataryxs in a directory.

    Args:
        dir_path: Directory containing .dataryx pickles
    ...
    dataryxs = list(dir_path.glob("**/*.dataryx"))

    if not dataryxs:
        print(f"No .dataryx files found in {dir_path}")
        return []

    print(f"Found {len(dataryxs)} dataryx(s) to migrate\n")

    migrated = []
    failed = []

    for dataryx in dataryxs:
        # Preserve directory structure
        relative = dataryx.relative_to(dir_path)
        suffix = ".yaml" if format == "yaml" else ".json"
        output_path = output_dir / relative.with_suffix(suffix)
        output_path.parent.mkdir(parents=True, exist_ok=True)

        try:
            migrate_dataryx(dataryx, output_path, format)
            migrated.append(output_path)
        except Exception as e:
            print(f"✗ Failed: {dataryx.name} - {e}")
            failed.append((dataryx, e))
