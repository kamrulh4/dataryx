...
class TestReceivedTableTransformation:
    ...
    def test_csv_flat_to_nested(self, temp_dir):
        ...
        pickle_path = temp_dir / 'test.dataryx'
        with open(pickle_path, 'wb') as f:
            pickle.dump(flow, f)

        output_path = migrate_dataryx(pickle_path, format='json')
...
class TestRoundTrip:
    ...
    def test_complex_flow_roundtrip(self, temp_dir):
        ...
        pickle_path = temp_dir / 'complex.dataryx'
        with open(pickle_path, 'wb') as f:
            pickle.dump(flow, f)

        output_path = migrate_dataryx(pickle_path, format='yaml')

        # Load and validate YAML
        with open(output_path) as f:
            data = yaml.safe_load(f)

        # Verify DataryxData format
        assert data['dataryx_version'] == '2.0'
        assert data['dataryx_name'] == 'complex_flow'
        assert data['dataryx_id'] == 1
