...
class DesignerView(ft.Container):
    ...
    def update_config_ui(self):
        ...
        if node.node_type == "explore_data":
            ...
            if not mappings:
                mappings.append(JoinMap(left_col="", right_col=""))

            grid_mappings = ft.Column(spacing=6)
            mapping_rows = []
            ...
            def save_join_config(e):
                ...
                for l_dd, r_dd in mapping_rows:
                    if l_dd.value and r_dd.value:
                        final_mappings.append(JoinMap(left_col=l_dd.value, right_col=r_dd.value))
                        
                if not final_mappings:
                    self.show_dialog("Error", "Please select key columns for join mapping.")
                    return
